...
def download_excel() -> tuple[BytesIO | None, date | None]:
    ...
    try:
        ...
        print(f"❌ 日期解析失敗 ({date_part!r}): {e}")
        return None, None

    print(f"📅 Excel 資料日期: {file_date}")
...
def parse_holdings(raw: BytesIO) -> pd.DataFrame:
    ...
    Excel 欄位：A=股票代號, B=股票名稱, C=股數, D=持股權重
    """
    wb = load_workbook(raw, data_only=True)
    ws = wb.active
    ...
    for row in ws.iter_rows(min_row=21, min_col=1, max_col=4, values_only=True):
        ...
        if not code:
            break
        code_str   = str(code).strip().rstrip("*")   # 去掉星號
...
def save_txt(df: pd.DataFrame, file_date: date) -> str:
    ...
    if os.path.exists(save_path):
        print(f"ℹ️ 檔案已存在，將覆寫: {save_path}")
    ...
    for i, row in df.iterrows():
        ...
        prices.append(price)
        print(f"  [{i+1:>2}/{total}] {sid} {row['股票名稱']}: {price}")
...
def main():
    ...
    # 比對資料日期是否為當天
    if file_date != today:
        print(f"⛔ Excel 資料日期 ({file_date}) 不是今天 ({today})，結束。")
        return

    print("✅ 日期符合，開始解析持股資料...")
    df = parse_holdings(raw)
    if df.empty:
        print("❌ 解析結果為空，請確認 Excel 格式。")
        return

    print(f"   共 {len(df)} 筆持股")
